...
const MAP_COLOR_PENALTY_HEX = '#ef4444';
const MAP_COLOR_WARNING_HEX = '#f59e0b';
const MAP_COLOR_NORMAL_HEX = '#22c55e';
...
function afrToColor(v:number, min:number, max:number, penaltyLow?:number, warningHigh?:number){
  if (penaltyLow !== undefined && v < penaltyLow) {
    return MAP_COLOR_PENALTY_HEX;
  }
  if (penaltyLow !== undefined && warningHigh !== undefined && v >= penaltyLow && v < warningHigh) {
    return MAP_COLOR_WARNING_HEX;
  }
...
export class LoggerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private readonly mapSegmentMinMoveDeg = 0.000003; // ~0.3m latitude movement
  private lastChartRenderSignature = '';
  ...
  loadAndApplyConfig(mode: 'live' | 'history', raceId: number): void {
    const sub = this.eventService.getRaceConfigSnapshot(raceId, mode).subscribe({
      next: (response) => {
        ...
        if (mode === 'history' && this.configSource !== 'snapshot') {
          console.warn('[Logger] history mode config source is not snapshot:', this.configSource, 'raceId=', raceId);
        }
        ...
        const afrWarningHigh = toNum((cfg as any).afr_warning_high, 16.0);
        this.afrWarningHigh = afrWarningHigh > this.afrLimit ? afrWarningHigh : this.afrLimit + 0.1;
  ...
  private updateChartFromGlobalArray(): void {
    ...
    const points = this.getOptimizedDisplayPoints(this.isHistoryMode ? raw : raw.slice(-this.CHART_LIVE_MAX_POINTS));
    if (!points?.length) return;

    const nextSignature = this.buildChartRenderSignature(points);
    if (this.displayOptimizationEnabled && nextSignature === this.lastChartRenderSignature) {
      return;
    }
    this.lastChartRenderSignature = nextSignature;
  ...
  private getOptimizedDisplayPoints(points: TelemetryPoint[]): TelemetryPoint[] {
    if (!this.displayOptimizationEnabled || points.length <= 2) {
      return points;
    }
    ...
    for (let i = 1; i < points.length - 1; i++) {
      ...
      const currentAfr = Number(current.afr);
      const lastAfr = Number(lastKept.afr);
      const afrClose = Number.isFinite(currentAfr) && Number.isFinite(lastAfr)
        ? Math.abs(currentAfr - lastAfr) <= this.chartDedupeAfrEpsilon
        : current.afr == null && lastKept.afr == null;
      const timeClose = (current.ts - lastKept.ts) <= this.chartDedupeTimeWindowMs;

      if (afrClose && timeClose) {
        continue;
      }
  ...
  private buildChartRenderSignature(points: TelemetryPoint[]): string {
    if (!points.length) {
      return '';
    }
    ...
    const last = points[points.length - 1];
    const norm = (v: number | undefined): string => {
      const n = Number(v);
      ...
      if (isLineMode) {
        ...
        for (let i = 0; i < pts.length - step; i += step) {
          ...
          const afr  = afrA!=null && afrB!=null ? (afrA + afrB)/2
                      ...
                      : afrB!=null ? afrB
                      : (afrMin + afrMax)/2;
          const color = afrToColor(afr, afrMin, afrMax, this.afrLimit, this.afrWarningHigh);
 ...
 this.detailOpts = {
  ...
  private buildAfrAnnotations(): ApexAnnotations {
    const clampWarning = this.afrWarningHigh > this.afrLimit ? this.afrWarningHigh : this.afrLimit + 0.1;
    const transformY = (value: number): number => {
      if (!this.afrGraphInverted) {
        return value;
      }
      return this.afrGraphsMinLimit + this.afrGraphsMaxLimit - value;
    };

    return {
      yaxis: [
        {
          y: transformY(this.afrLimit),
          borderColor: '#ef4444',
          strokeDashArray: 2,
          label: {
            borderColor: '#ef4444',
            ...
            text: `Penalty Low: ${this.afrLimit.toFixed(2)}`,
            position: 'right',
            offsetX: 5,
          }
        },
        {
          y: transformY(clampWarning),
          borderColor: '#f59e0b',
          strokeDashArray: 2,
          label: {
            borderColor: '#f59e0b',
            ...
            text: `Warning High: ${clampWarning.toFixed(2)}`,
            position: 'right',
            offsetX: 5,
          }
        }
  ...
  private deckIngestPoint(point: TelemetryPoint): void {
    ...
    if (this.ringBufferCount > 0) {
      ...
      const prevLng = this.targetPositions[prevIdx * 2];
      const prevLat = this.targetPositions[prevIdx * 2 + 1];

      // Update segment: source = previous point position, target = current point position (already stored above)
      this.sourcePositions[segIdx * 2] = prevLng;
      this.sourcePositions[segIdx * 2 + 1] = prevLat;
      // Note: target for this segment is already stored in targetPositions[writeIdx] above

      const isTinyMove = this.displayOptimizationEnabled
        && Math.abs(lng - prevLng) <= this.mapSegmentMinMoveDeg
        && Math.abs(lat - prevLat) <= this.mapSegmentMinMoveDeg;
      ...
      this.segmentColors[segIdx * 4 + 3] = isTinyMove ? 0 : 255; // Alpha
    }
  ...
  private afrToColorUint8(afr: number): [number, number, number] {
    const severity = this.getAfrMapSeverity(afr);
    if (severity === 'penalty') {
      return [255, 0, 0];
    }
    if (severity === 'warning') {
